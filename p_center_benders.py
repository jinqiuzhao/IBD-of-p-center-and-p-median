...
def Benders_Decomposition(MP, eps=0.00001):
    ...
    while abs(UB - LB) / (LB + 0.0001) > eps:
        ...
        if Gap < 10:
            ub_obj, int_obj = add_benders_cut(MP_relax, y_val, LB, int_UB, int_sol=False, MP_int=MP)
        else:
...
def add_benders_cut(MP, y_val, mp_obj, ub, cb=False, cbcut=False, int_sol=True, updata=True, MP_int=None):
    ...
    for mm in range(Cus_n):
        ...
        if k > 0 and Cut_index[j, k_th] == 0 and c_dis[sort_index[k]] >= mp_obj:
            if cb:
                if c_dis[sort_index[k]] >= obj:
                    MP.cbLazy(w + lhs >= c_dis[sort_index[k]])
                    Cut_index[j, k_th] = 1
                # print(f"{w} + {lhs} >= {c_dis[sort_index[k]]}")
            elif cbcut:
...
def Benders_solve():
    ...
    LB, UB = Benders_Decomposition(org_model)
