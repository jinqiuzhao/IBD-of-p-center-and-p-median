...
def solve_MP(MP, callback=None, print_sol=False):
    ...
    if MP.isMIP:
        ...
        lb = MP.ObjVal
        if MP.Status != 2:
            lb = np.ceil(max(MP.ObjBound, MP.PoolObjBound))
...
def build_original_MIP():
    ...
    for j in range(Cus_n):
        # lhs = z - Dis_m[i, :] @ x[i]
        lhs = z - quicksum(Dis_m[i, j] * x[i][j] for i in range(Fac_n))  # 椭圆
        org_model.addConstr(lhs >= 0)
    # 一个点至少被a个设施覆盖的p-center
    #     for i in range(Fac_n):
    #         org_model.addConstr(z - Dis_m[i, j] * x[i][j] >= 0)

    org_model.addConstr(y.sum() == Fac_L)
    for j in range(Cus_n):
        org_model.addConstr(quicksum(x[i][j] for i in range(Fac_n)) == 2)   # 选择两个
    for i in range(Fac_n):
...
def add_benders_cut(MP, y_val, lb, ub, relax_ub=np.inf, cb=False, cbcut=False, int_sol=True, updata=0, MP_int=None,
    ...
    float_obj_j = 2 * int_obj_j2 - np.sum(np.multiply(a_matrix, y_sort), axis=1)
    float_obj = np.max(float_obj_j)
    if not int_sol and not cbcut:
        part_obj_j = float_obj_j[float_obj_j > lb]  #= np.ceil(lb + 0.00001)]
        num = int(min(np.ceil(len(part_obj_j) * 0.1), 10))   # 10 选取的割数量，直选目标值大于下界的10个
        if num <= 0:
            return float_obj, int_obj, constr_num
        max_ub = np.partition(part_obj_j, -num)[-num:].min()  # 这10个中最小的
        ...
        num = int(min(np.ceil(len(part_obj_j) * 0.5), 20))   # 20
        max_ub = np.partition(part_obj_j, -num)[-num:].min()

        part_obj_j = part_obj_j[part_obj_j <= ub]
        num = int(min(np.floor(len(part_obj_j) * 0.5), 20))  # 20 # int(min(np.floor(len(part_obj_j)-1), 20))  # int(min(np.floor(len(part_obj_j) * 0.5), 20))  # 20
        if num > 0:
            cut_ub = np.partition(part_obj_j, num)[:num].max()
        else:
            cut_ub = ub
        ...
        # part_obj_j = int_obj_j[int_obj_j > np.ceil(lb)]
        # part_obj_j = int_obj_j[int_obj_j > np.ceil(lb)]
        # num = int(min(len(part_obj_j), 30))  #int(len(part_obj_j))  #
        # if num <= 0:
        #     max_ub = np.ceil(lb) + 1
        # else:
        #     max_ub = np.partition(part_obj_j, -num)[-num:].min()
    ...
    for j in cut_set:
        ...
        # while 2 * y_dis[j, k-1] >= min(ub, relax_ub): # + y_dis[j, k-2]
        #     k = k-1
        ...
        lhs = quicksum((y_dis[j, k] - y_dis[j, m]) * y[Sort_dis_index[j, m]] for m in range(k) if (y_dis[j, k] - y_dis[j, m]) > 0)
        # if int_sol and not cb:
        ...
        #         a_cof[m] > 0)
        # if int_sol and not cb and k_js1[j] >= 2 and 2 * int_obj_j1[j] >= ub:  # > lb: #
        #     lhs_enhanced = quicksum((int_obj_j1[j] - y_dis[j, m]) * y[Sort_dis_index[j, m]] for m in range(k_js1[j]) if (int_obj_j1[j] - y_dis[j, m]) > 0)
        if MP_int is not None:
            lhs2 = quicksum((y_dis[j, k] - y_dis[j, m]) * y_1[Sort_dis_index[j, m]] for m in range(k) if (y_dis[j, k] - y_dis[j, m]) > 0)
        ...
        # if 2 * int_obj_j2[j] > np.ceil(lb):  # and (obj == obj_j or obj_j > ub):  # k > 0 and Cut_index[j, k_th] == 0 and
        if int_obj_j[j] > np.ceil(lb): # 2 * y_dis[j, k] > np.ceil(lb): #   # 2 * y_dis[j, k] > np.ceil(lb):  #
            if cb:
                if Cut_index[j, k_th] == 0:
                    MP.cbLazy(w + lhs >= 2 * y_dis[j, k]) # 2 * Sort_dis[k_th])
                    # Cut_index[j, k_th] = 1
                # Cut_index[j, k_th] = 1
                ...
                if Cut_index[j, k_th] == 0:
                    ...
                    MP.cbCut(w + lhs >= 2 * y_dis[j, k])   #Sort_dis[k_th])
                # print(f"customer {j}: {w} + {lhs} >= {int_obj_j[j]}")
            else:
                # MP.addConstr(w + lhs >= 2 * Sort_dis[k_th], name="cut_" + str(j) + "_" + str(k_th))

                MP.addConstr(w + lhs >= y_dis[j, k] * 2, name="cut_" + str(j) + "_" + str(k_th))
                # if int_sol and not cb:
                ...
                if MP_int is not None and np.ceil(lb) < int_obj_j[j]:  #  <= ub:
                    ...
                    # MP_int.addConstr(w_1 + lhs2 >= 2 * Sort_dis[k_th], name="cut_" + str(j) + "_" + str(k_th))
                    MP_int.addConstr(w_1 + lhs2 >= 2 * y_dis[j, k], name="cut_" + str(j) + "_" + str(k_th))
                    # print("add")
                    # Cut_index[j, k_th] = 1
                    # constr_num += 1
                    # print(f"customer {j}: {w} + {lhs2} >= {2 * int_obj_j2[j]}")
    if constr_num <= 0 and int_sol and not cb and ub - lb <= 1:  # constr_num <= 0 and
        cut_set = np.where(int_obj_j > lb)[0]
        for j in cut_set:
            lhs5 = quicksum(a_matrix[j, m] * y[Sort_dis_index[j, m]] for m in range(k_js1[j]) if a_matrix[j, m] > 0 and m != k_js1[j])
    ...
    if updata > 0 or (int_obj < ub and int_sol):
        ...
        for a in indices:
            ...
            if constr is not None:
                ...
                # 获得约束中非零系数的最小值
                # linExpr = MP.getRow(constr)
                # min_coeff = GRB.INFINITY
                # for i in range(linExpr.size()):
                #     coeff = linExpr.getCoeff(i)
                #     if coeff != 0 and coeff < min_coeff:
                #         min_coeff = coeff
                # if constr.RHS >= 2 * min(int_obj, ub):  # - min_coeff
                #     for d_k in range(k+1, len(Cut_index[j,:])):
                #         if Cut_index[j, d_k] > 0:
                #             delet_constr = MP.getConstrByName(f"cut_{j}_{d_k}")
                #             if delet_constr is not None:
                #                 MP.remove(delet_constr)
                #                 Cut_index[j, d_k] = 0
                #                 d_u += 1
                if constr.RHS <= lb:
                    MP.remove(constr)
                    Cut_index[j, k] = 0
                    d_l += 1
        print(f"delete lb {d_l}, delete ub {d_u}")
...
def solve_MIP():
    ...
    orig_model.setParam('timeLimit', 3600)
    opt_time = time.time()
    orig_model.optimize()
...
def Benders_solve():
    ...
    for m in range(0, org_model.SolCount):
        ...
        if m == 0:
            ...
            _, ub, constr_num = add_benders_cut(org_model, y_star, LB, UB, updata=0)
            UB = min(UB, ub)
            print(constr_num)
            break
    # org_model.update()
    ...
    while abs(UB - LB) / (LB + 0.001) > 0.001:
        ...
        if ub <= LB:
            ...
            UB = ub
            no_change_cnt = 0
            org_model.setParam(GRB.Param.Cutoff, ub)
            # org_model.addConstr(org_model.getVarByName(f"w") <= ub)
            # org_model.update()
        inter += 1
        no_change_cnt += 1
        if no_change_cnt >= 1:
            org_model.setParam('TimeLimit', HARD_TIME * np.ceil(no_change_cnt / 2))
        org_model.setParam('OutputFlag', 0)
        if constr_num <= 0:
            # no_change_cnt += 1
            org_model.setParam('TimeLimit', 3600)
            # org_model.setParam('MIPFocus', 3)
            org_model.setParam('OutputFlag', 1)
        # elif constr_num > 0:
        #     no_change_cnt = 0
        if time.time() - opt_time >= 3600:
            break
    print()
    # print(facility)
    return UB, LB, opt_time, inter, LB_0, facility
    # return UB, LB, opt_time, 0, 0


def load_data(data_type, data_set=None, fac_n=None, read_file=False):
    """
    data_type:
    ...
    if data_type == 1:
        ...
        instance = Loc_Data()
        instance.read_tsp(data_path, read_file)
...
if __name__ == "__main__":
    ...
    for i in data_sets:
        for f_n in fac_number:
            instance = load_data(data_type, data_set=i, fac_n=f_n, read_file=True)
            # Fac_L = 4
            ...
            #     for mm in range(10):
            #         seed = np.random.randint(0, 1000000, 1)[0]
            #         r = test.test_demand_change(Dis_m, facility=facility, endurance=UB,
